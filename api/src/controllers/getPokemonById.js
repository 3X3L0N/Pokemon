...
const getPokemonById = async (req, res) => {
  try {
    ...
    if (!Number(id)) {
      //esto es para que se fije si es id sacado de la api o de la db
      ...
      if (!dbPokemon)
        return res
          .status(404)
          .json({ message: `No existe Pokemon con id ${id}` });
    ...
    // si es numero  se hace un get a la API
    ...
    return res
      .status(500)
      .json({
        message: `Error al buscar el Pokemon con ese id || Error: ${error.message}`,
      });
  }
};
