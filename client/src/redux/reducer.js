...
const initialState = {
  ...
  allTypes: [],
  myFavorites: [],
  numberTypes: 1, // para actualizar una parte de los formularios de creacion
  pokemonsdb: [],
};
//La función reducer toma dos argumentos, Dentro se usa una declaración switch para manejar diferentes tipos de acciones y UPDATEAR el estate
  ...
  switch (action.type) {
    ...
    case REMOVE_POKEMON:
      return {
        ...
        PokeSinFiltro: state.PokeSinFiltro.filter(
    ...
    case POKE_NAME:
      return {
        ...state,
        Pokemons:
          action.payload === "data" ? state.PokeSinFiltro : action.payload,
      };
    ...
    case FILTER:
      ...
      allPokemonsFiltered =
        ...
        action.payload.Type === "all"
          ? allPokemonsFiltered
          : allPokemonsFiltered.filter((elem) =>
              elem.types.includes(action.payload.Type)
            );
    ...
    case ADD_FAV:
      const pokeFav = state.PokeSinFiltro.filter(
        (elem) => elem.id === action.payload
      );
      return {
        ...state,
    ...
    case DB_POKEMONS:
      return {
        ...state,
        pokemonsdb: state.PokeSinFiltro.filter(
          (elem) => typeof elem.id === "string"
        ),
      };
